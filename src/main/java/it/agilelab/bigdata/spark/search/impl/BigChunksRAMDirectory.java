 ...
 */
public class BigChunksRAMDirectory extends BaseDirectoryShim implements Accountable, Serializable {
  private static final long serialVersionUID = 1L;
  protected final Map<String,BigChunksRAMFile> fileMap = new ConcurrentHashMap<>();
  ...
  public BigChunksRAMDirectory(LockFactory lockFactory) {
    super(lockFactory);
  }
   ...
   */
  public BigChunksRAMDirectory(FSDirectory dir, IOContext context) throws IOException {
  ...
  public final long fileLength(String name) throws IOException {
    ensureOpen();
    BigChunksRAMFile file = fileMap.get(name);
    if (file == null) {
  ...
  public void deleteFile(String name) throws IOException {
    ensureOpen();
    BigChunksRAMFile file = fileMap.remove(name);
    if (file != null) {
      file.directory = null;
  ...
  public IndexOutput createOutput(String name, IOContext context) throws IOException {
    ensureOpen();
    BigChunksRAMFile file = newRAMFile();
    BigChunksRAMFile existing = fileMap.remove(name);
    if (existing != null) {
      sizeInBytes.addAndGet(-existing.sizeInBytes);
      existing.directory = null;
    }
    fileMap.put(name, file);
    return new BigChunksRAMOutputStream(name, file, true);
  }
  ...
  /**
   * Returns a new {@link BigChunksRAMFile} for storing data. This method can be
   * overridden to return different {@link BigChunksRAMFile} impls, that e.g. override
   * {@link BigChunksRAMFile#newBuffer(int)}.
   */
  protected BigChunksRAMFile newRAMFile() {
    return new BigChunksRAMFile(this);
  }
  ...
  public void renameFile(String source, String dest) throws IOException {
    ensureOpen();
    BigChunksRAMFile file = fileMap.get(source);
    if (file == null) {
  ...
  public IndexInput openInput(String name, IOContext context) throws IOException {
    ...
    return new BigChunksRAMInputStream(name, file);
  }
