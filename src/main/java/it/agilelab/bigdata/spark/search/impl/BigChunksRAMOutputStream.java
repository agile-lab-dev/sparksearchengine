...
public class BigChunksRAMOutputStream extends IndexOutput implements Accountable {
  ...
  public BigChunksRAMOutputStream() {
    this("noname", new BigChunksRAMFile(), false);
  }

  /** Creates this, with no name. */
  public BigChunksRAMOutputStream(BigChunksRAMFile f, boolean checksum) {
    this("noname", f, checksum);
  }

  /** Creates this, with specified name. */
  public BigChunksRAMOutputStream(String name, BigChunksRAMFile f, boolean checksum) {
    // FIXME: should be super("RAMOutputStream(name=\"" + name + "\")", name);, but constructor is protected
	super("RAMOutputStream", name);
    file = f;
